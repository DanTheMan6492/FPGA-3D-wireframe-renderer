...
// =============================================================================
`timescale 1ns / 1ps
// This file is being built incrementally. The current cut implements all the
// ALWAYS-ON / PERSISTENT logic - clock generation, UART intake, button
// debouncing + edge detection driving the rotation-speed registers, memory
// instantiations, and display_top wiring. The frame-pipeline FSM, the
// mat_mul arbitration mux, the framebuffer write mux, and the per-state
// memory address muxes are left as TODOs marked clearly below. As they get
// filled in, the always-on portion of this file should not need to change.
//
// See docs/top_level.md for the full architecture and docs/state_diagrams.md
// for per-state connection diagrams.
// =============================================================================
...
module renderer_top #(
    ...
    //
    // Note: in TDPRAM, port A has a single address that serves both read
    // and write. wireframe_gen's RMW pattern (present addr, wait 1 cycle,
    // read douta, then on a later cycle present same addr with wea=1)
    // works fine here.
    // =========================================================================
    ...
    // SUBMODULE INSTANCES
    //
    // Each submodule's memory-port outputs go to its own dedicated wires.
    // These wires will be selected by the per-state memory muxes (TODO).
    // Memory port INPUTS (e.g. read_data) come from the muxed `*_rd_data`
    // wires, since whichever module is reading the memory must see the
    // memory's port-B output.
    //
    // The mat_mul instance is shared between MVP_matrix_maker (during
    // TRANSFORM_MVP) and the inline vertex-batch FSM (TODO, during
    // TRANSFORM_VERT). Both sides of the mat_owner mux are declared here;
    // the vertex-batch side is currently tied off pending the FSM.
    // =========================================================================
    ...
    // ---- int_to_fp16 (used by inline SWAP FSM - TODO) -----------------------
    // The SWAP FSM will route shadow_mem byte reads through this converter
    // before writing them into obj_mem. For now its input is tied to
    // shadow_mem's read output (which is otherwise unused until SWAP runs).
    ...
    // =========================================================================
    // ILA - in-circuit debug probe (optional, enabled with `define ILA_DEBUG)
    //
    // The ila_0 module is generated by Vivado's IP Catalog (IP -> ILA).
    // Configure it with 8 probes of these widths:
    //   probe0 = 3   (state)
    //   probe1 = 8   (vertex_count)
    //   probe2 = 8   (face_count)
    //   probe3 = 1   (vblank_rise)
    //   probe4 = 1   (new_data_flag)
    //   probe5 = 1   (obj_wr_en)
    //   probe6 = 11  (obj_wr_addr)
    //   probe7 = 16  (obj_rd_data)
    //
    // This instantiation only references the generated module; Vivado links
    // it at synthesis. No module declaration is written by hand - the IP
    // core provides it. The whole block is compiled out unless ILA_DEBUG is
    // defined, so normal builds and iverilog simulation are unaffected.
    // =========================================================================
`ifdef ILA_DEBUG
    ila_0 u_ila (
        .clk    (clk),
