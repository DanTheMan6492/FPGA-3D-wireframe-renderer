...
Emits two files in $readmemh format:
  sin_lut.mem   — 256 entries, one fp16 sine value per line
  cos_lut.mem   — 256 entries, one fp16 cosine value per line

Usage:
  python3 gen_sincos_lut.py            # writes ../data/sin_lut.mem, ../data/cos_lut.mem
  python3 gen_sincos_lut.py out_dir    # writes to <out_dir>/sin_lut.mem, etc.
"""
...
if __name__ == "__main__":
    ...
    sins, coss = build()
    sp = os.path.join(out_dir, "sin_lut.mem")
    cp = os.path.join(out_dir, "cos_lut.mem")
    with open(sp, "w") as f:
        for v in sins: f.write(f"{v:04x}\n")
    with open(cp, "w") as f:
        for v in coss: f.write(f"{v:04x}\n")
    print(f"wrote sin_lut.mem and cos_lut.mem ({len(sins)} entries each) to {out_dir}")
